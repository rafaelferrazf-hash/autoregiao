  ...
  return (
    <main style={{ fontFamily: "'DM Sans', sans-serif", background: "#F7F6F3", minHeight: "100vh" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "#fff", borderBottom: "1px solid #E8E6E1", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#7A7670" }}>
          ...
          <span>›</span>
          <Link href="/veiculos" style={{ color: "#7A7670", textDecoration: "none" }}>Onix</Link>
          <span>›</span>
      ...
      {/* CONTEÚDO */}
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "80px 24px 48px", display: "grid", gridTemplateColumns: "1fr 340px", gap: 24, alignItems: "start" }}>
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ position: "relative", height: 380, borderRadius: 14, overflow: "hidden", marginBottom: 10, border: "1.5px solid #E8E6E1" }}>
              <Image src="/sem-foto.png" alt="Foto do veículo" fill style={{ objectFit: "cover" }} sizes="100vw" />
              <span style={{ position: "absolute", top: 14, left: 14, background: "#E85D26", color: "#fff", fontSize: 11, fontWeight: 500, padding: "4px 12px", borderRadius: 20 }}>⭐ Em Destaque</span>
              <span style={{ position: "absolute", bottom: 14, right: 14, background: "rgba(0,0,0,0.55)", color: "#fff", fontSize: 11.5, padding: "4px 10px", borderRadius: 6 }}>📷 {fotoAtiva + 1} / 6</span>
              <button onClick={() => setFotoAtiva(Math.max(0, fotoAtiva - 1))} style={{ position: "absolute", top: "50%", left: 12, transform: "translateY(-50%)", width: 36, height: 36, background: "rgba(255,255,255,0.85)", border: "none", borderRadius: 8, fontSize: 14, cursor: "pointer" }}>‹</button>
              <button onClick={() => setFotoAtiva(Math.min(5, fotoAtiva + 1))} style={{ position: "absolute", top: "50%", right: 12, transform: "translateY(-50%)", width: 36, height: 36, background: "rgba(255,255,255,0.85)", border: "none", borderRadius: 8, fontSize: 14, cursor: "pointer" }}>›</button>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 8 }}>
              {fotos.map(i => (
                <div key={i} onClick={() => setFotoAtiva(i)} style={{ position: "relative", height: 64, borderRadius: 8, overflow: "hidden", border: fotoAtiva === i ? "2px solid #E85D26" : "1.5px solid #E8E6E1", cursor: "pointer" }}>
                  <Image src="/sem-foto.png" alt={`Foto ${i + 1}`} fill style={{ objectFit: "cover" }} sizes="100px" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 500, padding: "3px 9px", borderRadius: 4, background: "#F7F6F3", color: "#7A7670", border: "1px solid #E8E6E1" }}>Publicado há 2 dias</span>
            </div>
            <h1 style={{ fontFamily: "Georgia, serif", fontSize: 28, fontWeight: 800, color: "#1A1917", marginBottom: 6 }}>Chevrolet Onix LT 1.0 Turbo</h1>
            <p style={{ fontSize: 14, color: "#7A7670", marginBottom: 14 }}>2022 · 38.000 km · Flex · Automático · Prata</p>

            {/* PREÇO */}
            <div style={{ display: "flex", alignItems: "flex-end", gap: 16, padding: 16, background: "#fff", border: "1.5px solid #E8E6E1", borderRadius: 12, marginBottom: 16 }}>
              <div>
                <div style={{ fontFamily: "Georgia, serif", fontSize: 32, fontWeight: 800, color: "#1A1917", lineHeight: 1 }}>R$ 72.900</div>
                <div style={{ fontSize: 13, color: "#7A7670", marginTop: 4 }}>ou 60x de R$ 1.580 · Taxa 1,49% a.m.</div>
              </div>
              <div style={{ marginLeft: "auto", textAlign: "right" }}>
                <div style={{ fontSize: 10, color: "#7A7670", letterSpacing: 0.5, textTransform: "uppercase" }}>Tabela FIPE</div>
                <div style={{ fontFamily: "Georgia, serif", fontSize: 15, fontWeight: 700, color: "#16A34A", marginTop: 2 }}>↓ 4% abaixo</div>
          ...
          <div style={{ background: "#fff", border: "1.5px solid #E8E6E1", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)" }}>
              {[["📅", "Ano", "2022/2022"], ["📍", "Quilometragem", "38.000 km"], ["⛽", "Combustível", "Flex"],
                ...
                <div key={label} style={{ padding: "14px 16px", borderRight: (i + 1) % 3 !== 0 ? "1px solid #E8E6E1" : "none", borderBottom: i < 6 ? "1px solid #E8E6E1" : "none" }}>
                  <div style={{ fontSize: 18, marginBottom: 4 }}>{icon}</div>
                  <div style={{ fontSize: 10.5, color: "#7A7670", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 2 }}>{label}</div>
                  <div style={{ fontFamily: "Georgia, serif", fontSize: 14, fontWeight: 700, color: "#1A1917" }}>{value}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: "#fff", border: "1.5px solid #E8E6E1", borderRadius: 12, marginBottom: 16, overflow: "hidden" }}>
            ...
            <div style={{ padding: "16px 18px", fontSize: 14, lineHeight: 1.7, color: "#1A1917" }}>
              ...
              <p style={{ marginBottom: 10 }}>Pneus novos, freios revisados, ar condicionado gelando. Interior impecável, sem manchas ou rasgos. Vidros e travas elétricas em todos os quatro.</p>
            ...
            <div style={{ padding: "14px 18px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {["Ar-condicionado", "Direção elétrica", "Vidros elétricos", "Travas elétricas", "Airbag duplo", "ABS", "Central multimídia", "Bluetooth", "Câmera de ré", "Sensor de estacionamento", "Faróis de neblina", "Rodas de liga"].map(item => (
                <div key={item} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#1A1917" }}>
                  <span style={{ fontSize: 14, color: "#16A34A" }}>✅</span> {item}
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: "#fff", border: "1.5px solid #E8E6E1", borderRadius: 12, overflow: "hidden" }}>
            ...
            <div style={{ padding: "16px 18px" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 500, color: "#7A7670", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 5 }}>Valor de entrada</div>
                  <input type="number" value={entrada} onChange={e => setEntrada(e.target.value)} style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #E8E6E1", borderRadius: 7, fontSize: 14, color: "#1A1917", background: "#F7F6F3", outline: "none", boxSizing: "border-box" }} />
                </div>
                <div>
                  <div style={{ fontSize: 10.5, fontWeight: 500, color: "#7A7670", textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 5 }}>Prazo</div>
                  <select value={prazo} onChange={e => setPrazo(e.target.value)} style={{ width: "100%", padding: "9px 12px", border: "1.5px solid #E8E6E1", borderRadius: 7, fontSize: 14, color: "#1A1917", background: "#F7F6F3", outline: "none", appearance: "none", boxSizing: "border-box" }}>
        ...
        <div style={{ position: "sticky", top: 76, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: "#fff", border: "1.5px solid #E8E6E1", borderRadius: 14, overflow: "hidden" }}>
            ...
            <div style={{ padding: "12px 18px", borderTop: "1px solid #E8E6E1", display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontSize: 11.5, color: "#7A7670", display: "flex", alignItems: "center", gap: 4 }}>👁️ 342 visualizações</span>
              <span style={{ fontSize: 11.5, color: "#7A7670", display: "flex", alignItems: "center", gap: 4 }}>🕐 Responde em ~1h</span>
            </div>
          </div>
          ...
          <div style={{ background: "rgba(22,163,74,0.08)", border: "1.5px solid rgba(22,163,74,0.15)", borderRadius: 12, padding: "14px 16px" }}>
            <div style={{ fontFamily: "Georgia, serif", fontSize: 13, fontWeight: 700, color: "#16A34A", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>🛡️ Compre com segurança</div>
            {["Loja verificada pela AutoRegião", "Anúncio com fotos reais", "Negocie direto com o lojista", "Solicite laudo cautelar antes de comprar"].map(item => (
